...
# KHO DỮ LIỆU ĐỀ THI CỐ ĐỊNH THEO PHOM ĐỀ KHẢO SÁT CHUẨN (CHỐNG LỖI MẤT NGỮ CẢNH)
VSTEP_EXAM_DB = {
    "1️⃣ VSTEP Nghe": [
        {
            "id": 1,
...
# CHỈ THỊ ÉP ĐỊNH DẠNG TUYỆT ĐỐI 3 DÒNG INTERLINEAR CHO TOÀN BỘ CÁC THÀNH PHẦN TIẾNG ANH
MASTER_PROMPT = """
# ROLE & PERSONALITY
...
2. Every question stem and all multiple-choice options (A, B, C, D) MUST follow the strict 3-line interlinear format (ENG, IPA, VIE) independently.
    ...
    if not is_submitted:
        ...
        if st.button("🚀 XÁC NHẬN NỘP BÀI TỰ LUẬN", use_container_width=True):
            if not api_key:
                ...
                Đóng gói phần chấm điểm, mẹo huấn luyện cấu trúc câu xương cá và các bài tập thực hành bổ sung (đúng cấu trúc 3 dòng có audio kèm theo) vào bên trong cặp thẻ [DIEN_GIAI_START] và [DIEN_GIAI_END].
                """
                with st.spinner("Hệ thống đang thẩm định bài viết luận..."):
...
# THUẬT TOÁN ĐỒ HỌA HOÁN ĐỔI MÀU SẮC ĐÁP ÁN ĐÚNG SANG XANH LỤC SAU KHI NỘP BÀI (HỖ TRỢ CHỌN PHƯƠNG ÁN TỰ ĐỘNG)
if st.session_state.current_section in ["1️⃣ VSTEP Nghe", "2️⃣ VSTEP Đọc"]:
    if not is_submitted:
        user_choice = st.radio(
            "Chọn phương án trả lời của thầy cô:", 
            ["-- Chọn câu trả lời / Choose your answer --"] + display_options, 
            key=f"radio_mcq_{active_q['id']}_{selected_de}"
        )
        # Nếu người học chọn câu trả lời hợp lệ, hệ thống tự động khóa và tính toán ngay lập tức
        if user_choice != "-- Chọn câu trả lời / Choose your answer --":
            ...
            Cuối cùng đóng gói phần bóc tách sơ đồ tư duy, từ khóa vàng định vị thính giác và phần CÂU HỎI THAM KHẢO BIẾN THỂ (mỗi câu bọc trong cặp thẻ AUDIO_START và AUDIO_END để tạo audio phát âm) vào cặp thẻ [DIEN_GIAI_START] và [DIEN_GIAI_END]. Mọi từ tiếng Anh xuất hiện trong bài tập bổ sung phải có phiên âm và dịch nghĩa 3 dòng tuyệt đối.
            """
            if not api_key:
                st.sidebar.error("Vui lòng nhập mã cấu hình hệ thống.")
            else:
                with st.spinner("Hệ thống đang đối chiếu dữ liệu..."):
                    try:
                        ...
                        st.error(f"Lỗi đường truyền dữ liệu: {e}")
    else:
...
if audio_data is not None:
    if st.sidebar.button("🚀 NỘP BÀI THI NÓI VSTEP", use_container_width=True):
        if not api_key:
            ...
            Gói toàn bộ sơ đồ cấu trúc câu Mind Map phân nhánh và ma trận câu hỏi tham khảo phát triển năng lực (áp dụng quy tắc 3 dòng cho tất cả các từ) vào cặp thẻ [DIEN_GIAI_START] và [DIEN_GIAI_END].
            """
            with st.spinner("Hệ thống đang xử lý và phân tách thính giác ngôn ngữ..."):
                try:
                    genai.configure(api_key=api_key)
                    model = genai.GenerativeModel(MODEL_NAME, system_instruction=MASTER_PROMPT, safety_settings=SAFETY_SETTINGS)
                    contents = [{
                    ...
                    st.session_state.submitted_state[q_key] = "Đã nộp dữ liệu âm thanh giọng nói thành công."
                    st.session_state.saved_explanations[q_key] = res_text
                    st.session_state.mic_key += 1
                    st.rerun()
                except Exception as e:
